XLFDT	;ISC-SF/STAFF - Date/Time Functions ;03/27/2003  14:09
	;;8.0;KERNEL;**71,120,166,168,179,280**LOCAL**;Jul 10, 1995
	;
	; **LOCAL MOD** Change History
	;
	; 2012 10 04 KBAZ/ZAG & VEN/TOAD: Add extrinsic functions $$HTS and
	; $$STH to match naming convention of other date/time conversions. Also
	; reorganize functions within routine to make them easier to find.
	; Untangle $$HTE and $$FMTE. Untangle FMDIFF and HDIFF. Add EOR.
	;
...
FMDIFF(X1,X2,X3)	;FM diff in two dates. if X3=1 in days, if X3=2 in seconds.
	...
	S X=(X1-%H) S:X3>1 X=X*86400+(X1(1)-$P(%H,",",2))
...
SEC(%)	;Convert $H to seconds.
	...
	;
	;
STH(S)	; Convert from seconds to $H
	Q (S\86400)_","_(S#86400)
	;
	;
